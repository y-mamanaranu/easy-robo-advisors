...
class Portfolio():
    ...
    def update_accounts(self, options:Options=None):
        ...
        if self.use_SBI:
            self.__update_SBI(options)
        if self.use_Monex:
            self.__update_Monex(options)
        if self.use_bitFlyer:
            self.__update_bitFlyer(options)
    ...
    def __update_SBI(self, options):
        self.SBI = SBIHandler(options=options)
        self.SBI.update()
        self.SBI.close()
        for i, row in self.SBI.df.iterrows():
            if row.valuation == 0:
                continue
            self.portfolio[row.ticker]["amount"] += row.amount
            self.portfolio[row.ticker]["valuation"] += row.valuation
    
    def __update_Monex(self, options):
        self.Monex = MonexHandler(options)
        self.Monex.update()
        self.Monex.close()
        for i, row in self.Monex.df.iterrows():
            if row.valuation == 0:
                continue
            self.portfolio[row.ticker]["amount"] += row.amount
            self.portfolio[row.ticker]["valuation"] += row.valuation
    
    def __update_bitFlyer(self, options):
        if not (self.bitflyer_api_key and self.bitflyer_api_secret):
            print("Need `self.bitflyer_api_key` and `self.bitflyer_api_secret`.")
    ...
    def update_target_from_csv(self, path):
    ...
    def update_propose(self):
        self.__update_per_month()
        self.__update_per_day()
        self.__update_per_dayn(20)
    
    @staticmethod
    def curve_per_month(weight, valuation, total, income, delay, tor):
        ...
        for i, item in filter.iteritems():
            if item is True:
                ...
                per_month.iloc[i] -= np.sign(per_month.iloc[i]) * tor_month.iloc[i]

        return per_month * income / per_month.sum()
        
    def __update_per_month(self):        
        total = self.df["valuation"].sum()

        self.df["per_month"] = self.curve_per_month(
            self.df.weight, 
            self.df.valuation, 
            total, 
            self.income,
        ...
        currency_to_prepare = np.unique(self.df.currency[self.df.currency == self.df.currency].values)
        for c in currency_to_prepare:
            self.df.loc[self.df.ticker==c, "per_month"] += self.df.per_month[self.df.currency==c].sum()
        self.df["per_month_amount"] = self.df.per_month * self.df.amount / self.df.valuation
        
        filter = (self.df.per_month == self.df.per_month)
        self.df.loc[filter, 'per_month'] = self.df[filter].per_month.apply(int)

        filter = (self.df.dtype == "int") & (self.df.per_month_amount == self.df.per_month_amount)
        self.df.loc[filter, 'per_month_amount'] = self.df[filter].per_month_amount.apply(int)
        
    def __update_per_day(self):
        ...
        filter = (self.df.dtype == "int") & (self.df.per_day_amount == self.df.per_day_amount)
        self.df.loc[filter, 'per_day_amount'] = self.df[filter].per_day_amount.apply(int)
    
    def __update_per_dayn(self, n):
        ...
        self.df.loc[filter, f"per_day{n}"] = self.df[filter][f"per_day{n}"].apply(int)
        
        filter = (self.df.dtype == "int") & (self.df[f"per_day{n}_amount"] == self.df[f"per_day{n}_amount"])
        self.df.loc[filter, f"per_day{n}_amount"] = self.df[filter][f"per_day{n}_amount"].apply(int)
    
    @property
    def propose_columns(self):
        return ["ticker", "amount", "valuation", "per_month", "per_month_amount", "per_day", "per_day_amount", "per_day20", "per_day20_amount"]
    
    def propose(self, account):
        return self.df[self.df.account == account][self.propose_columns]
    ...
    def plot(self, ax, value, title=None):
        ...
        iner_labels = list(self.type_detail.keys())
        iner_vals = np.array([vals[self.df.type == label].sum() for label in iner_labels])
        iner_labels_per = [f"{label}\n{val:1.1f}%" for label, val in zip(iner_labels, 100 * iner_vals / iner_vals.sum())]

        outer_labels = [detail for label in iner_labels for detail in self.type_detail[label]]
        outer_vals = np.array([vals[self.df.detail == detail].sum() for detail in outer_labels])
        outer_labels_per = [f"{label} {val:1.1f}%" for label, val in zip(outer_labels, 100 * outer_vals / outer_vals.sum())]
        ...
        outer_colors = [c for i, label in enumerate(iner_labels) for c in color(cmap, i, self.type_detail[label])]
        ...
        iner_donut = ax.pie(
            ...
            colors=iner_colors, 
            labeldistance=0.6,
            textprops={"horizontalalignment": "center", "color": "w", "weight": "bold"}, 
        )

        wedges, texts = outer_donut
        ...
        for i, p in enumerate(wedges):
            ...
            ax.annotate(outer_labels_per[i], xy=(x, y), xytext=(x_text, y_text),
